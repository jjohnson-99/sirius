...
//!
//! Each arm's sink owns the `"union_{i}"` port name it feeds: `resolve_port_id` hands back a
//! `string_view` into that member which the wiring descriptor and `next_port_info` retain for the
//! life of the query, so the storage has to live on a plan-tree node.
//!
...
class sirius_physical_passthrough_sink : public sirius_physical_operator {
 public:
  ...
  //! The `"union_{i}"` label this arm feeds; read by `resolve_port_id`.
