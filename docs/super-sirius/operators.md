...
- **`PASSTHROUGH_SINK -> UNION` is `PARTIAL`.** `resolve_barrier`'s fall-through is `FULL`, which
...
- **Owns its port name.** `resolve_port_id` returns a `string_view` into `_union_port_label`, and
