...
- **`PASSTHROUGH_SINK -> UNION` is `PARTIAL`**, not `resolve_barrier`'s `FULL` fall-through, which
...
- **Emits `pipelineable_operator_data`, not `partitioned_operator_data`.** With no `partition_idx`
  ...
  UNION task to GPU 0.
- **Owns its port name.** `resolve_port_id` returns a `string_view` into `_union_port_label`, which
  the wiring descriptor and `next_port_info` retain for the life of the query.
